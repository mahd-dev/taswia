var iosync = (function(){
  ...
  var _socket = io.connect(location.origin);
  ...
  jsonpatch.observe(_data, function (patch) {
    ...
    patch.forEach(function (p) {
      ...
      for (var i = 0; i < _ignore.length; i++) {
        if(deepCompare(p,_ignore[i])){ // patch is from server so prevent loopback to server
      ...
      if(!ign) client_patches.push(p); // if patch don't exists in server patches si it's a client patch
    });
    ...
    _observers.forEach(function (o) {
      ...
      if(patches.length) {
        o.observer(patches); // trigger the observer
      }
    });
  ...
  return{
    bind: function (path, observer) {
      ...
      var val=xPath_get(path); // is that path exists
      if(val) return val; // so return it's val for "reference binding"
      else {
        var v = xPath_create(path); // create an empty var temporary until the server send us the content
        _socket.emit("bind", path); // notify server to provide us lastest value, and upcoming updates
        return v; // return it also for "reference binding"
      }
    },
  ...
  function xPath_get(path) {
    ...
    keys.forEach(function (k) {
      if(k!=""){ // escape empty keys
        if(val.hasOwnProperty(k)) val=val[k]; // okay, jump to that key
  ...
  function xPath_create(path) {
    ...
    keys.forEach(function (k) {
      if(k!=""){ // escape empty keys
        if(!val.hasOwnProperty(k)) val[k]={}; // oups, key does not exists, so create it
        val=val[k]; // okay, jump to that key
      }
    });
